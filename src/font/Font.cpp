...
void Font::print(const char* const a_Letters, int const a_X, int const a_Y, float const a_Scale, glm::vec3 a_Color) {
	...
	// ОТЛАДКА: выводим информацию о шрифте
	...
	while (index < maxIndex && a_Letters[index] != '\0') {
		...
		if (iteratorLetter != Chars.end()) {
			...
			if (letter.width <= 0 || letter.height <= 0) {
				// Даже для пустых символов продвигаем позицию
			...
			// 1. Нормализуем координаты правильно
			...
			// 2. Вычисляем позицию символа с учётом offset'ов
			...
			// 3. ПРАВИЛЬНЫЕ вершины для квада (против часовой стрелки)
			GLfloat verts[6][4] = {
				// Первый треугольник (верхний левый -> нижний правый -> верхний правый)
				{ charLeft,  charTop,    uvLeft,  uvTop    },     // верхний левый
				{ charRight, charBottom, uvRight, uvBottom },     // нижний правый  
				{ charRight, charTop,    uvRight, uvTop    },     // верхний правый

				// Второй треугольник (верхний левый -> нижний левый -> нижний правый)
				{ charLeft,  charTop,    uvLeft,  uvTop    },     // верхний левый
				{ charLeft,  charBottom, uvLeft,  uvBottom },     // нижний левый
				{ charRight, charBottom, uvRight, uvBottom }      // нижний правый
			};
			...
			xPos += myPage.base * a_Scale * 0.3f; // Меньший отступ для неизвестных символов
		}
